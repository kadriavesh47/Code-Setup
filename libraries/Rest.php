...
class REST
{
    ...
    public function api_key($key, $name = 'key')
	{
    ...
    public function webapi($url,$data)
    {

        $this->api_key('JVP8xGk4hsX2cZd0L3NQwYbI0mf4exPiSoAhVYnz', 'key');
        $conf['server']     = API_URL;
        $conf['http_user']  = HTTP_USER;
        $conf['http_pass']  = HTTP_PASS;
        $conf['http_auth']  = HTTP_AUTH;
        $this->initialize($conf);
        $action = $this->post($url,$data);
        return $action;
    }
}
